...
def generate_auth_headers(api_key: str, api_secret: str, payload: dict = None) -> Dict[str, str]:
    timestamp = str(int(time.time() * 1000))
    recv_window = "5000"
    
    # Bybit signature string (timestamp + api_key + recv_window + param_str)
    # Simple GET payload support for preflight
    param_str = ""
    if payload:
        param_str = "&".join([f"{k}={payload[k]}" for k in sorted(payload.keys())])
                ...
                try:
                    ...
                    async with session.request(method, url, **kwargs) as resp:
                        if resp.status == 200:
                            ...
                            # [GEKTOR v14.8.3] Capture Exchange Clock (Zero-Latency sync)
                            self.last_server_time = int(data.get("time", 0))
