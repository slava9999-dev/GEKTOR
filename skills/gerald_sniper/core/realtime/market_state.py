...
from .models import SymbolLiveState, ZeroGCOrderbook, L1Tick
...
class LiveMarketState:
    ...
    def __init__(self):
        ...
        self.is_replaying = False # [REPLAY MUTEX v12.0]
    ...
    def update_trade(self, symbol: str, price: float, qty: float, side: str, ts_ms: int, exchange: str = "bybit", staleness_ms: float = 0.0):
        ...
        state.l2_radar.apply_trade(price, qty)
        
        # [GEKTOR v21.5] VPIN Integration (Macro-Accumulation)
        if state.vpin_engine is None:
            # Heuristic: 1/50th of a typical $50M turnover ~ $1M bucket
            # Fallback for small assets: $100k
            # TODO: Pull turnover from UniverseShaker in next iteration
            state.vpin_engine = MacroVPINTracker(bucket_volume=1_000_000.0)
            
        v_tick = RawTick(timestamp=now, price=price, volume=qty * price)
        vpin = state.vpin_engine.ingest_tick(v_tick)
        
        if vpin is not None:
             state.last_vpin = vpin
             # [Audit 25.5] Signal broadcast to AlertBus
             ...
             asyncio.create_task(bus.publish(SignalEvent(
                 symbol=symbol,
                 source="VPIN",
                 signal_type="MACRO_ACCUMULATION",
                 confidence=vpin,
                 direction="LONG" if vpin > 0.8 else "SHORT", # Placeholder logic
                 price=price,
                 metadata={"vpin": vpin}
             )))
    ...
    def update_orderbook(self, symbol: str, bids_usd: float, asks_usd: float, 
        ...
        if is_snapshot:
            ...
            state.l2_radar.apply_delta(bids or [], asks or []) # Radar handles snapshots as deltas too
        else:
            state.orderbook.apply_delta({"b": bids, "a": asks, "u": update_id})
            state.l2_radar.apply_delta(bids or [], asks or [])
            
        self._last_update_ts[key] = state.last_l2_ts = now
        self._stats["updates"] += 1
        
        # [GEKTOR v14.9] Trigger JIT OFI after L2 processing
        bb = state.orderbook.best_bid
        ba = state.orderbook.best_ask
        if bb and ba:
            tick = L1Tick(
                timestamp=now,
                price=state.current_price or (bb + ba) / 2,
                bid_p=bb,
                bid_v=state.orderbook._price_map["b"].get(bb, 0.0),
                ask_p=ba,
                ask_v=state.orderbook._price_map["a"].get(ba, 0.0)
            )
            state.current_ofi = state.ofi_engine.update_and_get(tick)
    ...
    def update_bba(self, symbol: str, bid: float, ask: float, ts_ms: int, update_id: int = 0, exchange: str = "bybit"):
        ...
        # [GEKTOR v14.9] JIT Microstructural Evaluation (OFI) on L1 BBA change
        tick = L1Tick(
            timestamp=now,
            price=state.current_price or (bid + ask) / 2,
            bid_p=bid,
            bid_v=state.orderbook._price_map["b"].get(bid, 0.0),
            ask_p=ask,
            ask_v=state.orderbook._price_map["a"].get(ask, 0.0)
        )
        state.current_ofi = state.ofi_engine.update_and_get(tick)
