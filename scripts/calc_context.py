...
def get_system_ram():
    ...
    try:
        ...
        total = sum(int(l) for l in lines) / (1024 ** 3)  # GB
        return total
    except:
...
def calculate_context_limits():
    ...
    for name, info in models.items():
        ...
        kv_per_token = 2 * info["num_layers"] * info["num_kv_heads"] * info["head_dim"] * 2
        ...
        available_kv_gb = available_ram_gb - info["model_size_gb"]  # model weights first
    ...
    print()
    print("=" * 60)
    print(f"💡 RECOMMENDATION for qwen2.5-coder:14b:")
