...
def check_chroma():
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            print(
                f"✅ ChromaDB: heartbeat OK ({data.get('nanosecond_heartbeat', 'alive')})"
            )
            return True
    except Exception:
        ...
        try:
            subprocess.run(
                ["docker", "start", "chroma"], capture_output=True, timeout=15
            )
            time.sleep(3)
            req = urllib.request.Request("http://localhost:8000/api/v2/heartbeat")
            with urllib.request.urlopen(req, timeout=5) as resp:
...
def check_bridge():
    ...
    # Compute base dir dynamically (scripts/ is one level down)
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    status_file = os.path.join(base_dir, "bridge", "status.json")
    try:
        ...
        try:
            ...
            print(
                f"❌ Bridge: Invalid timestamp format in status.json: {last_sync_str}"
            )
            return False
        ...
        if data.get("status") == "running" and diff < 60:
            ...
            print(
                f"❌ Bridge: Inactive or Stale (Status: {data.get('status')}, Last sync: {int(diff)}s ago)"
            )
            return False
    except json.JSONDecodeError:
        print("❌ Bridge: status.json is corrupt or empty")
        return False
    except Exception as e:
...
def main():
    ...
    if all_ok:
        ...
        reasons = []
        if not checks["GPU"]:
            reasons.append("GPU temp high")
        if not checks["Bridge"]:
            reasons.append("Bridge daemon inactive")
