...
def test_inference():
    ...
    try:
        with urllib.request.urlopen(req, timeout=120) as resp:
            ...
            print(
                f"   Model loaded: {'yes' if data.get('load_duration') else 'was cached'}"
            )
            assert True
...
def check_vram_for_128k():
    ...
    print("=" * 50)
    print("  Model size:     ~9 GB (Q4_K_M)")
    print("  Available VRAM: 8 GB (RTX 4070 Laptop)")
    print("  128k KV cache:  ~20-64 GB (depends on layer offloading)")
    print()
