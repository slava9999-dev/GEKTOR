...
def analyze_logs():
    ...
    for line in content.split('\n'):
        if "ERROR" in line.upper() or "EXCEPTION" in line.upper() or "FAIL" in line.upper():
            errors.append(line)
...
def main():
    ...
    if recent_errors:
        print(f"[{datetime.now().isoformat()}] ⚠️ Errors found! Notifying Gerald and Slava...")
        
        # 1. Notify Slava via Telegram (Real-time alert)
        try:
            guard_path = os.path.join(BASE_DIR, "skills", "telegram-guard", "guard.py")
            subprocess.run([sys.executable, guard_path, "System Errors Detected! Check logs for details."], capture_output=True)
        except:
            pass
...
{recent_errors}
..."""
        msg_path = os.path.join(BRIDGE_INBOX, f"auto_fix_{int(datetime.now().timestamp())}.msg")
        with open(msg_path, 'w', encoding='utf-8') as f:
            f.write(improvement_msg)
        print(f"✅ Reflector: Improvement request sent to inbox.")
    else:
        print(f"✅ Reflector: System healthy, no critical errors found.")
